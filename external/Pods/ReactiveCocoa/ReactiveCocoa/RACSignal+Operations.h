...
/// Initially sends `skipCount` number of `next`s,
/// then sends `next`s only if we don't receive another `next` in `throttleInterval` seconds.
///
/// If `skipCount` is specified (i.e., != 0), resulting signal sends `skipCount` number of `next`s, then switches to throttle behaviour.
- (RACSignal *)throttle:(NSTimeInterval)throttleInterval afterAllowing:(NSUInteger)skipCount;

/// Initially sends `skipCount` number of `next`s,
/// then sends `next`s only if we don't receive another `next` in `throttleInterval` seconds or `strikeInterval` exceeds.
///
/// If `skipCount` is specified (i.e., != 0), resulting signal sends `skipCount` number of `next`s, then switches to throttle behaviour.
///
/// During throttling, signal sends `next` every time `strikeInterval` passes since last successfuls `next` sent.
/// Generally, `strikeInterval` should be longer than `throttleInterval`, otherwise all `next`s will be sent
