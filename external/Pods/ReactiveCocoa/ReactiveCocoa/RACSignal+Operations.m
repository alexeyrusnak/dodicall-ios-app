...
- (RACSignal *)throttle:(NSTimeInterval)throttleInterval afterAllowing:(NSUInteger)skipCount {
    
    __block uint numberEventsPassedThrough = 0;
    
    return [self throttle:throttleInterval
        valuesPassingTest:^BOOL(id next) {
            numberEventsPassedThrough++;
            
            if(numberEventsPassedThrough <= skipCount)
                return NO;
            else
                return YES;
        }];
...
- (RACSignal *)throttle:(NSTimeInterval)throttleInterval afterAllowing:(NSUInteger)skipCount withStrike:(NSTimeInterval)strikeInterval {
    NSCParameterAssert(strikeInterval >= 0);
    
    __block uint numberEventsPassedThrough = 0;
    __block NSDate *lastReceiveTime;
    __block NSDate *lastForwardTime;
    ...
    return [self throttle:throttleInterval
        valuesPassingTest:^BOOL(id next) {
            ...
            if(numberEventsPassedThrough == 1)
                lastForwardTime = now;
            
            if(numberEventsPassedThrough <= skipCount) {
                //Forward values before throttling
                lastReceiveTime = now;
                lastForwardTime = now;
                return NO;
            }
            ...
            else if(numberEventsPassedThrough > 1){
                //Don't forward very first value
                //Start Throttling
                
                NSTimeInterval timeSinceLastReceive = [now timeIntervalSinceDate:lastReceiveTime];
                lastReceiveTime = now;
                
                if(timeSinceLastReceive > throttleInterval) {
                    //Throttle interval exceeded, forward value
                    lastForwardTime = now;
                    return NO;
                }
                else {
                    //Throttle NOT exceeded, check strike
                    NSTimeInterval timeSinceLastForward = [now timeIntervalSinceDate:lastForwardTime];
                    if(timeSinceLastForward >= strikeInterval) {
                        //Strike interval exceeded, forward Value
                        lastForwardTime = now;
                        return NO;
                    }
                }
